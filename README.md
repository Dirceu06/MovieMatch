# 🎬 MovieMatch - Tinder for Movies

[![Python](https://img.shields.io/badge/Python-3.11+-blue.svg)](https://python.org)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.104+-green.svg)](https://fastapi.tiangolo.com)
[![Streamlit](https://img.shields.io/badge/Streamlit-1.28+-red.svg)](https://streamlit.io)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-15+-blue.svg)](https://postgresql.org)
[![License](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Live Demo](https://img.shields.io/badge/🚀-Live%20Demo-success)]((https://moviematch-dejota.streamlit.app))


**MovieMatch** é um sistema de recomendação de filmes com funcionalidades sociais, estilo Tinder.

> ✨ **Live Demo:** [Frontend]((https://moviematch-dejota.streamlit.app)) | [API Docs]((https://moviematch-6k2e.onrender.com/docs))

## ✨ Funcionalidades

### 🎯 Sistema de Recomendação
- **Recomendações personalizadas** baseadas nos gêneros favoritos do usuário
- **Interface estilo Tinder** com 👍/👎 para avaliar filmes
- **Filtros avançados**: ano, popularidade, filmes brasileiros
- **Integração em tempo real** com a API do TMDB

### 👥 Rede Social
- **Sistema de amigos** - adicione e acompanhe amigos
- **Filmes em comum** - veja quais filmes você e seus amigos gostaram
- **Perfil personalizado** com histórico de filmes assistidos

### 🔐 Segurança
- **Autenticação JWT** com tokens de acesso e refresh
- **Senhas criptografadas** com bcrypt

### **Infraestrutura**
- **Render.com** - Hosting da API (Free tier)
- **Streamlit Cloud** - Hosting do frontend (Free)
- **Neon.tech** - PostgreSQL como serviço (Free tier)
